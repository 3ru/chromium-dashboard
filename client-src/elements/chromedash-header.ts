...
export class ChromedashHeader extends LitElement {
  ...
  @property({type: String}) googleSignInClientId = '';
  @property({type: String}) devMode = '';
  ...
  render() {
    return html`
      <header data-testid="header">
        ...
        <nav>
          ${!IS_MOBILE && !this.loading ? html`<div class="flex-container">${this.renderAccountMenu()}</div>` : nothing}
        </nav>
      </header>
    `;
  }
